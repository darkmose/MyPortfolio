...
namespace DestroyIt
{
    public class Explode : MonoBehaviour
    {
        ...
        public void Explosion()
        {
            ...
            EventAggregator.Post(this, new GameTriggerEvent()
            {
                TriggerID = 0,
                TriggerType = GameTriggerType.Explosion,
        ...
        private void AddAffectedObjects(int colliderCount, ExplosiveDamage explosiveDamage, float chipAwayPercentage)
        {
            for (int i=0; i<colliderCount; i++)
            {
                ...
                if (col == GetComponent<Collider>())
                    continue;
                var ignoreLayersCheck = LayerTools.LayerIsInLayerMask(col.gameObject.layer, _ignoreLayers);
                if (ignoreLayersCheck)
                    continue;
                
                // Check for Rigidbodies
                Rigidbody rbody = col.attachedRigidbody;
                if (rbody != null && !affectedRigidbodies.Contains(rbody))
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawWireSphere(transform.position, nearBlastRadius);
            Gizmos.DrawWireSphere(transform.position, pointBlankBlastRadius);
            Gizmos.DrawWireSphere(transform.position, farBlastRadius);
        }
#endif
    }
}